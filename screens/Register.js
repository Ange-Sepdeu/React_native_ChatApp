...
function Register({navigation}) {
    ...
    const handleSubmit = async() => {
        if(name==='' || email==='') {
         return Toast.error("Fill the inputs!!!")   
            ...
            axios.post(`http://192.168.43.97:7000/register`, {name:name, email:email, password: pass})
            .then(res => {
                if(res.status===200) {
  ...
  return (
    ...
    style={styles.container}>
        ...
        <TextInput value={name} onChangeText={(text) => setName(text)} placeholder='Username' placeholderTextColor='#2790F1' style={styles.inputs}/>
        ...
        <TextInput value={pass} onChangeText={(text)=> setPassword(text)} secureTextEntry={true} icon={EmailIcon}  placeholder='Password' placeholderTextColor='#2790F1' style={styles.input3}/>
        <TouchableOpacity style={styles.button} onPress={handleSubmit}>
            <Text style={{fontSize: 20, color:'#FFFFFF', textAlign: 'center', lineHeight:25, top:5, fontWeight:'600'}} >
                Send</Text>
        </TouchableOpacity>
        <Text style={{color: 'white', top: 220, left: 150, fontSize:17}} onPress={()=>navigation.navigate('Login')}>Login</Text>
    </KeyboardAvoidingView>
    </>
  )
}
